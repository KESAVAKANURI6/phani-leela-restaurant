...
async function loadStats() {
  try {
    ...
    animateNumber('stat-categories', 7);
    ...
    grid.innerHTML = popular.map((d, i) => `
      <div class="popular-card">
        <img
          src="${getDishImg(d.id)}"
          alt="${d.name}"
          class="popular-dish-img"
          onerror="this.src='/images/hero-thali.png'"
        />
        <div>
...
const DISH_IMAGES = {
  ...
  'dish-011': '/images/butter-chicken.png',
  ...
  'dish-036': '/images/chicken-biryani.png',
  'dish-037': '/images/paneer-biryani.png',
  'dish-038': '/images/ice-cream.png'
};

function getDishImg(id, customUrl) {
  if (customUrl && customUrl.trim()) return customUrl.trim();
  const found = (allMenuItems || []).find(i => i.id === id);
  if (found && found.imageUrl && found.imageUrl.trim()) return found.imageUrl.trim();
...
async function updateTokenPreview() {
  ...
  if (!previewEl || !nameInput) return;

  if (nameInput.value.trim()) {
    try {
      const orders = await Api.getOrders();
      const nextToken = (orders || []).length + 1;
      previewEl.textContent = '#' + getTokenDisplay(nextToken);
    } catch(e) {
      previewEl.textContent = '#001';
    }
  } else {
...
function initTokenPreview() {
  ...
  const nameInput = document.getElementById('customerName');
  if (!nameInput) return;
  nameInput.addEventListener('input', updateTokenPreview);
  nameInput.addEventListener('focus', updateTokenPreview);
}
...
async function placeOrder() {
  ...
  // Validate Card Payment Details if Card method selected
  if (selectedPaymentMethod === 'CARD') {
    ...
    if (!cardNum) {
      showToast('⚠️ Please enter your Card Number', true);
      document.getElementById('cardNumber')?.focus();
      return;
    }
    if (cardNum.replace(/\s/g, '').length < 12) {
      showToast('⚠️ Please enter a valid Card Number', true);
      document.getElementById('cardNumber')?.focus();
      return;
    }
    if (!expiry) {
      showToast('⚠️ Please enter Card Expiry Date (MM/YY)', true);
      document.getElementById('cardExpiry')?.focus();
      return;
    }
    if (!cvc) {
      showToast('⚠️ Please enter 3 or 4-digit CVC code', true);
  ...
  try {
    ...
    const finalToken = res.tokenNumber || res.tableNumber || 1;
    ...
    showTokenModal(finalToken, name, selectedPaymentMethod, payStatus, totalAmount);
