...
public class RestaurantDataSeeder implements ApplicationRunner {
    ...
    // ── SAMPLE ORDERS ────────────────────────────────────────────────────────────
    private void createSampleOrders(Session session) {
        // Order 1 — Table T3: Butter Chicken + Garlic Naan + Sweet Lassi + Gulab Jamun
        session.run("MERGE (o:Order {id:'order-s001'}) SET o.tableNumber='T3', " +
                "o.customerName='Arjun Sharma', o.status='Served', " +
                "o.createdAt='2026-08-05T12:00:00', o.totalAmount=820, o.specialNote=''");
        session.run("MATCH (o:Order {id:'order-s001'}),(m:MenuItem {id:'dish-011'}) MERGE (o)-[:CONTAINS {quantity:1,subtotal:380}]->(m)");
        session.run("MATCH (o:Order {id:'order-s001'}),(m:MenuItem {id:'dish-020'}) MERGE (o)-[:CONTAINS {quantity:2,subtotal:140}]->(m)");
        session.run("MATCH (o:Order {id:'order-s001'}),(m:MenuItem {id:'dish-027'}) MERGE (o)-[:CONTAINS {quantity:2,subtotal:240}]->(m)");
        session.run("MATCH (o:Order {id:'order-s001'}),(m:MenuItem {id:'dish-023'}) MERGE (o)-[:CONTAINS {quantity:1,subtotal:120}]->(m)");

        // Order 2 — Table T7: 2× Chicken Biryani + 2× Sweet Lassi
        session.run("MERGE (o:Order {id:'order-s002'}) SET o.tableNumber='T7', " +
                "o.customerName='Priya Reddy', o.status='Served', " +
                "o.createdAt='2026-08-05T13:15:00', o.totalAmount=1000, o.specialNote=''");
        session.run("MATCH (o:Order {id:'order-s002'}),(m:MenuItem {id:'dish-015'}) MERGE (o)-[:CONTAINS {quantity:2,subtotal:760}]->(m)");
        session.run("MATCH (o:Order {id:'order-s002'}),(m:MenuItem {id:'dish-027'}) MERGE (o)-[:CONTAINS {quantity:2,subtotal:240}]->(m)");

        // Order 3 — Table T1: Paneer Butter Masala + Dal Makhani + 3× Butter Naan + 2× Rasmalai
        session.run("MERGE (o:Order {id:'order-s003'}) SET o.tableNumber='T1', " +
                "o.customerName='Rahul Verma', o.status='Served', " +
                "o.createdAt='2026-08-05T14:30:00', o.totalAmount=1140, o.specialNote=''");
        session.run("MATCH (o:Order {id:'order-s003'}),(m:MenuItem {id:'dish-006'}) MERGE (o)-[:CONTAINS {quantity:1,subtotal:320}]->(m)");
        session.run("MATCH (o:Order {id:'order-s003'}),(m:MenuItem {id:'dish-007'}) MERGE (o)-[:CONTAINS {quantity:1,subtotal:260}]->(m)");
        session.run("MATCH (o:Order {id:'order-s003'}),(m:MenuItem {id:'dish-019'}) MERGE (o)-[:CONTAINS {quantity:3,subtotal:180}]->(m)");
        session.run("MATCH (o:Order {id:'order-s003'}),(m:MenuItem {id:'dish-024'}) MERGE (o)-[:CONTAINS {quantity:2,subtotal:300}]->(m)");

        // Order 4 — Table T5: Mutton Rogan Josh + Mutton Biryani (still Preparing)
        session.run("MERGE (o:Order {id:'order-s004'}) SET o.tableNumber='T5', " +
                "o.customerName='Sneha Patel', o.status='Preparing', " +
                "o.createdAt='2026-08-05T17:45:00', o.totalAmount=900, o.specialNote='Extra spicy please'");
        session.run("MATCH (o:Order {id:'order-s004'}),(m:MenuItem {id:'dish-012'}) MERGE (o)-[:CONTAINS {quantity:1,subtotal:450}]->(m)");
        session.run("MATCH (o:Order {id:'order-s004'}),(m:MenuItem {id:'dish-016'}) MERGE (o)-[:CONTAINS {quantity:1,subtotal:450}]->(m)");

        // Order 5 — Table T9: Paneer Tikka + Palak Paneer + 2× Butter Naan + 2× Masala Chai
        session.run("MERGE (o:Order {id:'order-s005'}) SET o.tableNumber='T9', " +
                "o.customerName='Kavya Krishna', o.status='Served', " +
                "o.createdAt='2026-08-05T16:00:00', o.totalAmount=760, o.specialNote=''");
        session.run("MATCH (o:Order {id:'order-s005'}),(m:MenuItem {id:'dish-002'}) MERGE (o)-[:CONTAINS {quantity:1,subtotal:280}]->(m)");
        session.run("MATCH (o:Order {id:'order-s005'}),(m:MenuItem {id:'dish-008'}) MERGE (o)-[:CONTAINS {quantity:1,subtotal:290}]->(m)");
        session.run("MATCH (o:Order {id:'order-s005'}),(m:MenuItem {id:'dish-019'}) MERGE (o)-[:CONTAINS {quantity:2,subtotal:120}]->(m)");
        session.run("MATCH (o:Order {id:'order-s005'}),(m:MenuItem {id:'dish-029'}) MERGE (o)-[:CONTAINS {quantity:2,subtotal:120}]->(m)");

        logger.info("Created 5 sample orders across tables T1, T3, T5, T7, T9");
    }
}
